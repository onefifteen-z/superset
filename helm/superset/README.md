...
![Version: 0.22.9](https://img.shields.io/badge/Version-0.22.9-informational?style=flat-square)
...
To load the metadata database credentials from a Secret managed outside the chart
(for example one created by CloudNativePG or an AWS RDS integration), reference it
...
database:
  ...
  name: superset
  existingSecret: metadata-db-credentials
  secretKeys:
    password: password   # default
    user: username       # optional; omit to keep using database.user
```

`DB_PASS` (and `DB_USER` when `secretKeys.user` is set) are injected via `secretKeyRef`
into every Superset container and the init job, and are omitted from the chart's
generated `-env` Secret. The built-in `wait-for-postgres` init containers only need
`DB_HOST`/`DB_PORT`, which are still provided by the chart Secret. A full `database.uri`
takes precedence and disables the reference.
...
| database | object | `{"driver":null,"existingSecret":"","host":null,"name":null,"password":null,"port":null,"secretKeys":{"password":"password","user":""},"ssl":{"enabled":false,"mode":"require"},"uri":null,"user":null}` | Database connection configuration for the Superset metadata database |
...
| database.existingSecret | string | `""` | Existing Secret holding the database credentials (supports templating). When set, DB_PASS (and optionally DB_USER) are injected via secretKeyRef into every Superset container and the init job instead of being rendered into the chart's Secret. Ignored when database.uri is set. |
...
| database.secretKeys.password | string | `"password"` | Key in database.existingSecret holding the password |
| database.secretKeys.user | string | `""` | Optional key in database.existingSecret holding the username; when empty database.user is used |
